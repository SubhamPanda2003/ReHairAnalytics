...
async def download_file(
    ...
    if not record:
        # Derived images (e.g. change-map heatmaps) intentionally aren't rows in
        # db.images -- they're not captured photos, so they shouldn't show up in
        # galleries/delete/export. Allow a user to fetch only their own, scoped
        # by their own user_id baked into the path at creation time.
        own_derived_prefix = f"{store.APP_NAME}/changemaps/{user['user_id']}/"
        if not path.startswith(own_derived_prefix):
